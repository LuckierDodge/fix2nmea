...
class Transformer : public rclcpp::Node
{
 public:
  ...
  : Node("fix2nmea")
  {
	...
	nmea_pub_ = this->create_publisher<nmea_msgs::msg::Sentence>("/nmea", 10);  
	
      	
      	//Subscribes to obtain NavSatFix messages from topic /ublox_gps_node/fix
      	navsatfix_sub_ = this->create_subscription<sensor_msgs::msg::NavSatFix>("/fix", 10, std::bind(&Transformer::receiveNavSatFix, this, _1));
      	//navhpposllh_sub_ = this->create_subscription<ublox_msgs::msg::NavHPPOSLLH>("/fix", 10, std::bind(&Transformer::receiveNavHpPosLlh, this, _1));
  }
